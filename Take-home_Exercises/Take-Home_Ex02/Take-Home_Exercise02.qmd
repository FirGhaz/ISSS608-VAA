...
P7 <- P7 +
  stat_summary(aes(label = round(..y.., 2)), geom = "text", fun.y = mean, 
               vjust = -1.0, color = "red", size = 3) +
  stat_summary(aes(label = round(..y.., 2)), geom = "text", fun.y = median, 
               vjust = 1.8, color = "blue", size = 3) + theme(panel.background = element_rect(fill = "grey95", color = NA),
        panel.grid.major = element_line(color = "grey80", size = 0.5),
        panel.grid.minor = element_line(color = "grey90", size = 0.25))
