...
bmi_node_2021 <- data.frame(bmi_node_2021)
bmi_node_2021$LON <- as.numeric(as.character(bmi_node_2021$LON))
bmi_node_2021$LAT <- as.numeric(as.character(bmi_node_2021$LAT))

# Convert your data frame to an sf object
node_sf <- st_as_sf(bmi_node_2021, coords = c("LON", "LAT"), crs = 4326)
...
col.1 <- adjustcolor("orangered", alpha=0.015)
col.2 <- adjustcolor("yellow", alpha=0.015)
...
g <- ggraph(bmi2021_graph, layout = "sugiyama", circular = TRUE) +
  ...
  geom_node_point(aes(colour = continent), size = 2.5, shape=20) +
