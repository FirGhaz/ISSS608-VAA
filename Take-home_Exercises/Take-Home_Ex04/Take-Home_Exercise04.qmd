...
q + annotate("point", x = legend_x, y = legend_y_total_trade, shape = 21, size = 7, fill = "dimgrey", colour = "dimgrey") +
...
#ggsave("geotrade2.png", plot = q, width = 10, height = 6, dpi = 300)
```
To build Interactivity

```{r}
img <- png::readPNG("C:/FirGhaz/ISSS608-VAA/Take-home_Exercises/Take-Home_Ex04/images/geotrade.png")

# Convert the image to base64
img_base64 <- base64enc::dataURI(file = "C:/FirGhaz/ISSS608-VAA/Take-home_Exercises/Take-Home_Ex04/images/geotrade.png", mime = "image/png")
...
# Prepare the hover text with HTML tags for formatting
...
# Add the nodes as scattergeo trace
fig <- fig %>%
  add_trace(
    data = bmi_node_2021,
    type = 'scattergeo',
    ...
    marker = list(
      ...
      projection = list(type = 'robinson')# Adjust color as needed
    ),
    lon = ~LON,
    lat = ~LAT,
    dragmode = FALSE,
...
bmi_edges_2021$hover_text <- paste(
  "Origin: ", bmi_edges_2021$Origin,
  "<br>Main Partner: ", bmi_edges_2021$Partners
...
# Loop through each edge to populate the lists
for(i in 1:nrow(bmi_edges_2021)) {
  # Add source and target coordinates
  all_lons <- c(all_lons, bmi_edges_2021$source_lon[i], bmi_edges_2021$target_lon[i],1)
  all_lats <- c(all_lats, bmi_edges_2021$source_lat[i], bmi_edges_2021$target_lat[i],1)
  # Add hover text for the edge
  all_texts <- c(all_texts, bmi_edges_2021$hover_text[i], "", "")
}  
# Add the nodes as scattergeo trace
fig <- fig %>%
  add_trace(
  ...
  add_trace(
    ...
    text = all_texts,
    hoverinfo = "text",
    hoverlabel = list(bgcolor = "white", font = list(color = "orangered")),
    line = list(color = 'orangered', width = 0.00002, alpha = 0.1) # Customize line appearance here
...
ggraph(bmi2021_graph, layout = "fr") + #layout options = kk, fr, nicely,
...
set.seed(123)
g <- ggraph(bmi2021_graph, layout = 'kk') + #can  choose "fr", "kk", "circle", "graphopt"
...
# Calculate betweenness to visualize node importance
V(bmi2021_graph)$betweenness <- betweenness(bmi2021_graph)
