...
for(i in 1:nrow(bmi_edges_2021)) {
  # Define the hover text for the edge
  edge_hover_text <- with(bmi_edges_2021[i, ], paste("", Origin, 
                                                      "|", Partners))
 
  # Add edge as a line trace
  fig <- fig %>% 
    add_trace(
      type = 'scattergeo',
      mode = 'lines',
      lon = c(bmi_edges_2021$source_lon[i], bmi_edges_2021$target_lon[i]),
      lat = c(bmi_edges_2021$source_lat[i], bmi_edges_2021$target_lat[i]),
      line = list(color = 'yellow', width =0.001),
      hoverinfo = 'text',
