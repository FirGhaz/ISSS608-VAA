...
bmi_edges_2021$hover_text <- paste("Origin: ", bmi_edges_2021$Origin,
                                   "<br>Partner: ", bmi_edges_2021$Partners)

# Prepare data for edge traces
edges_lon <- unlist(mapply(function(x, y) c(x, y, NA), bmi_edges_2021$source_lon, bmi_edges_2021$target_lon))
edges_lat <- unlist(mapply(function(x, y) c(x, y, NA), bmi_edges_2021$source_lat, bmi_edges_2021$target_lat))
edges_hover_text <- bmi_edges_2021$hover_text
...
fig <- fig %>%
  add_trace(
    ...
    hoverinfo = "text",
    hoverlabel = list(bgcolor = "white", font = list(color = "black")),
    text = edges_hover_text,  # Hover text for edges
    line = list(color = "orange", width = 0.5)  # Customize line appearance
  )
